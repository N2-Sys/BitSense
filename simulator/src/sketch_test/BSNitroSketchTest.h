/**
 * @file BSNitroSketch.h
 * @author XierLabber
 * @brief Test BSNitroSketch
 *
...
#include <sketch/BSNitroSketch.h>

#define BSNS_PARA_PATH "NS.para"
#define BSNS_TEST_PATH "NS.test"
#define BSNS_DATA_PATH "NS.data"
#define BSNS_BS_PATH "NS.bs"

namespace OmniSketch::Test {

/**
 * @brief Testing class for BSNitroSketch
 *
 */
template <int32_t key_len, int32_t no_layer, typename T,
          typename hash_t = Hash::AwareHash>
class BSNitroSketchTest : public TestBase<key_len, T> {
...
public:
   ...
   * '.')
   */
  BSNitroSketchTest(const std::string_view config_file)
      : TestBase<key_len, T>("Nitro Sketch with BS", config_file, BSNS_TEST_PATH) {}
...
void BSNitroSketchTest<key_len, no_layer, T, hash_t>::runTest() {
  /**
  ...
  parser.setWorkingNode(
      BSNS_PARA_PATH); // do not forget to to enclose it with braces
  ...
  parser.setWorkingNode(BSNS_DATA_PATH);
  ...
  parser.setWorkingNode(BSNS_BS_PATH);
  ...
  std::unique_ptr<Sketch::SketchBase<key_len, T>> ptr(
      new Sketch::BSNitroSketch<key_len, no_layer, T, hash_t>(
...
#undef BSNS_PARA_PATH
#undef BSNS_TEST_PATH
#undef BSNS_DATA_PATH
