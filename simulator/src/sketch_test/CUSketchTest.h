...
class CUSketchTest : public TestBase<key_len, T> {
...
public:
   ...
   * '.')
   */
  CUSketchTest(const std::string_view config_file)
      : TestBase<key_len, T>("CU Sketch", config_file, CU_TEST_PATH) {}
...
void CUSketchTest<key_len, T, hash_t>::runTest() {
  /**
  ...
  parser.setWorkingNode(
      CU_PARA_PATH); // do not forget to to enclose it with braces
  ...
  parser.setWorkingNode(CU_DATA_PATH);
  ...
  std::unique_ptr<Sketch::SketchBase<key_len, T>> ptr(
      new Sketch::CUSketch<key_len, T, hash_t>(depth, width));
