/**
 * @file WavingSketchTest.h
 * @author XierLabber (you@domain.com)
 * @brief Testing Waving Sketch
 *
...
#include <sketch/WavingSketch.h>

#define WS_PARA_PATH "WS.para"
#define WS_TEST_PATH "WS.test"
...
class WavingSketchTest : public TestBase<key_len, T> {
...
public:
   ...
   */
  WavingSketchTest(const std::string_view config_file)
      : TestBase<key_len, T>("Waving Sketch", config_file, WS_TEST_PATH) {}

  /**
   * @brief Test Waving Sketch
...
void WavingSketchTest<key_len, T, hash_t>::runTest() {
  /**
  ...
  ///
  int32_t bucket_num, heavy_part_length; // sketch config
  ...
  parser.setWorkingNode(
      WS_PARA_PATH); // do not forget to to enclose it with braces
  ...
  if (!parser.parseConfig(heavy_part_length, "heavy_part_length"))
    return;
  /// Step v. To know about the data, we  switch to [WS.data].
  parser.setWorkingNode(WS_DATA_PATH);
  ...
  std::unique_ptr<Sketch::SketchBase<key_len, T>> ptr(
      new Sketch::WavingSketch<key_len, T, hash_t>(bucket_num, heavy_part_length));
