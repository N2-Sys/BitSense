/**
 * @file PCMSketchTest.h
 * @author XierLabber (you@domain.com)
 * @brief Test PCM Sketch
 *
...
#include <sketch/PCMSketch.h>
#include <iostream>

#define PCM_PARA_PATH "PCM.para"
#define PCM_TEST_PATH "PCM.test"
#define PCM_DATA_PATH "PCM.data"

namespace OmniSketch::Test {

/**
 * @brief Testing class for PCM Sketch
 *
 */
template <int32_t key_len, typename T, typename hash_t = Hash::AwareHash>
class PCMSketchTest : public TestBase<key_len, T> {
...
public:
   ...
   * '.')
   */
  PCMSketchTest(const std::string_view config_file)
      : TestBase<key_len, T>("PCM Sketch", config_file, PCM_TEST_PATH) {}
...
void PCMSketchTest<key_len, T, hash_t>::runTest() {
  /**
  ...
  parser.setWorkingNode(
      PCM_PARA_PATH); // do not forget to to enclose it with braces
  ...
  parser.setWorkingNode(PCM_DATA_PATH);
  ...
  std::unique_ptr<Sketch::SketchBase<key_len, T>> ptr(
      new Sketch::PCMSketch<key_len, T, hash_t>(word_num, hash_num, lg_used_bits, pyramid_depth));
