/**
 * @file BSUnivMonTest.h
 * @author dromniscience (you@domain.com)
 * @brief Test BS-optimized UnivMon Sketch
 *
 * @copyright Copyright (c) 2022
 *
 */
#include <common/test.h>
#include <sketch/BSUnivMon.h>

#define BSUM_PARA_PATH "UM.para"
#define BSUM_TEST_PATH "UM.test"
#define BSUM_DATA_PATH "UM.data"
#define BSUM_BS_PATH "UM.bs"
...
class BSUnivMonTest : public TestBase<key_len, T> {
...
public:
   ...
   * '.')
   */
  BSUnivMonTest(const std::string_view config_file)
      : TestBase<key_len, T>("UnivMon with BS", config_file, BSUM_TEST_PATH) {
  }
...
void BSUnivMonTest<key_len, no_layer, T, hash_t>::runTest() {
  /**
  ...
  parser.setWorkingNode(
      BSUM_PARA_PATH); // do not forget to to enclose it with braces
  ...
  parser.setWorkingNode(BSUM_DATA_PATH);
  ...
  parser.setWorkingNode(BSUM_BS_PATH);
  ...
  std::unique_ptr<Sketch::SketchBase<key_len, T>> ptr(
      new Sketch::BSUnivMon<key_len, no_layer, T, hash_t>(
...
#undef BSUM_PARA_PATH
#undef BSUM_TEST_PATH
#undef BSUM_DATA_PATH
#undef BSUM_BS_PATH
